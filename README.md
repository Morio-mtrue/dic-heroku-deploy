...
- `SECRET_KEY` and `DEBUG` come from the environment. An unset
  `DJANGO_DEBUG` means debug is on, which keeps local development simple, so
  every production host must set `DJANGO_DEBUG=False`. `render.yaml` does
  this for Render.
